...
- `make test`, from the `verification/` directory, runs the automated Node.js
  test suite (see [Verification](#verification)).
...
OpenRouter stack). The numbered cases under each file below specify the
behavior covered by every automated test; they do not prescribe how that
behavior is tested, and cases are grouped by their current test file only to
make them easy to locate.

#### `verification/logic.test.js`

Extracts the pure, DOM-free helpers embedded inline
...
Covers: empty-spec validation (manual procedure step 6), the error message
contract, the `Network error` message (step 7), the blob document contents for
`html` and `markdown` (steps 3 & 5, at the content level), and the trust model
(step 10).
...
- every Markdown-result style declaration specified in
  [Success](#success), for body text, code and preformatted text,
...
Spins up a Caddyfile-mirroring origin (static
...
spec.
Covers: the page is served with the required controls (manual procedure
step 2), `marked` is loaded from a CDN with no local vendored copy (step 9),
method-not-allowed, the 422 validations and error shape (step 6), and the 200
html/markdown headers plus the exact request body/`Content-Type` (step 8).
Rejecting non-`POST` methods with `405` has no corresponding manual procedure
step below; it is only ever verified automatically.
...
Drives the page in a real (headless)
...
popup (manual procedure steps 3 & 9), the **back button** returns to the form
with the previous type and spec still entered (step 4), and a `<script>` in
rendered markdown output actually *executes* in the rendered tab (step 10 —
the trust model). To keep the suite hermetic, the page's request for the marked
...
Verifies source formatting: the page
(`implementation/src/index.html`) and every JavaScript file in
`verification/` must be prettier-formatted (default options), one test case
per file. `prettier` is
