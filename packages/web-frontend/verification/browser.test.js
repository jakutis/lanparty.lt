...
// ../specification/verification/verifying.md.
//
// Requires playwright-core (run `make install-browser`) and a Chromium
...
const skip = chromium
  ? false
  : "playwright-core is not installed (run `make install-browser`)";
...
const CDN_PATTERN = /^https:\/\/cdn\.jsdelivr\.net\/npm\/marked@\d+\.\d+\.\d+\/marked\.min\.js$/;
...
test("browser", { skip, concurrency: false }, async (t) => {
  ...
  await t.test("clicking Generate navigates the same tab to an HTML blob URL (steps 3, 9)", async () => {
    await page.goto(stack.base + "/");
    assert.match(cdnUrl || "", CDN_PATTERN, "the page requested marked from the required CDN URL");

    await page.fill("#spec", "a hello page");
    await page.click("#submit");
    await page.waitForURL((u) => String(u).startsWith("blob:"));

    assert.ok(page.url().startsWith("blob:"), "the tab navigated to a blob URL");
    assert.equal(context.pages().length, 1, "no new tab or popup was created");
    assert.equal(await page.textContent("h1"), "hello", "the generated HTML is rendered");
  });

  await t.test("the back button returns to the form with inputs retained (step 4)", async () => {
    await page.goBack();
    assert.equal(page.url(), stack.base + "/", "back returns to the form page");
    assert.ok(await page.isVisible("#form"), "the form is shown again");
    assert.equal(await page.inputValue("#spec"), "a hello page", "the previous spec is still entered");
    assert.equal(await page.inputValue("#type"), "html", "the previous type is still selected");
  });

  await t.test("a script in rendered markdown executes (step 10, trust model)", async () => {
