...
Each package's behavior is defined in `packages/<name>/specs/`, with `specs/main.md` as the entry point. The code in `packages/<name>/code/src/` must exhibit exactly the behavior the spec states — all of it, and nothing beyond it. The blackbox tests in `packages/<name>/code/test/` verify that it does.
